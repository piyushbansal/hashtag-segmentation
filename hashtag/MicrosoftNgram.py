...
class LookupService(object):
    def __init__(self, token='1210376f-6e06-4021-8cc0-f0bacbc2c898', model=None, serviceUri=None):
        self.token = token
        if (token is None):
...
lookup = LookupService(model = 'bing-body/apr10/1')
lookup_bi = LookupService(model = 'bing-body/apr10/2')

def result(word):
	print lookup.GetJointProbability(word)
